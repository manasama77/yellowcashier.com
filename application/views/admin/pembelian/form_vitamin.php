<script>
	$(document).ready(function(){
		$('#foto').on('change', function(){ readURL(this); });

		$('#barcode').on('input', function(){ checkKode(); });
		$('#kode').on('input', function(){ checkKode(); });
		$('#hpp').on('input', function(){ hitungMargin(); });
		$('#hpj').on('input', function(){ hitungMargin(); });
		$('#hpg').on('input', function(){ hitungMargin(); });
	});
	...
	function checkKode()
	{
		...
		if(barcode.length > 0 && kode.length > 0){
			$.ajax({
				...
				method: 'get',
				dataType: 'json',
				data: { barcode: barcode, kode: kode },
				beforeSend: function(){
